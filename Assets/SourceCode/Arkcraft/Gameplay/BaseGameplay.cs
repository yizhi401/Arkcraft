...
namespace Arkcraft.Gameplay
{
    ...
    {
        private string id;
        protected Arkcraft.World.ArkWorld world;
        ...
        public virtual void Init(Arkcraft.World.ArkWorld world)
        {
            this.world = world;
        }
        ...
        public void CalculateLastObjectId()
        {
            ...
            foreach (ACObject cwobject in world.avatarManager.Avatars)
                if (cwobject.objectId > lastObjectId)
                    lastObjectId = cwobject.objectId;

            foreach (ACObject cwobject in world.itemManager.Items)
                if (cwobject.objectId > lastObjectId)
                    lastObjectId = cwobject.objectId;

            foreach (ACObject cwobject in world.tileManager.DynamicTiles)
                if (cwobject.objectId > lastObjectId)
                    lastObjectId = cwobject.objectId;
